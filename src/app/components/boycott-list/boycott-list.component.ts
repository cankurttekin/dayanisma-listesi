...
export class BoycottListComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (!this.boycottItems || this.boycottItems.length === 0) {
      if (changes['categoryId'] || changes['groupId']) {
  ...
  assignRandomColorsToCards(): void {
    this.boycottItems.forEach((item) => {
      if (!this.cardColors.has(item.id)) {
        const randomColor =
          this.retroColors[Math.floor(Math.random() * this.retroColors.length)];
  ...
  loadItemsByCategory(): void {
    if (this.categoryId) {
      this.boycottService
        .getBoycottItemsByCategory(this.categoryId, this.groupId ?? undefined)
        .subscribe((items) => {
  ...
  searchItems(): void {
    if (this.searchQuery.trim()) {
      this.boycottService
        .searchBoycottItems(this.searchQuery)
        .subscribe((items) => {
          this.boycottItems = items;
          this.expandedItems.clear();
          this.assignRandomColorsToCards();
